...
//		$breakOutList = getBreakoutStock2 ($date, "55_DAY_HIGH", $testArray);
...
//print strftime('%c');
		...
		$result = queryMysql($query);
		$query  = "create table ".$simplePriceHistory." select symbol, trade_date, trade_date_id, open, high, low, close, ";
...
*/
		...
		$query = "select count(*) from price_history where symbol = 'AAPL' and trade_date >= '".$start_date."'";
		$result = queryMysql($query);
		...
		echo json_encode( $num_trade_days);
			...
			$stockRetArray = historical_stock_return($symbol, $start_date);
			...
			$stockRetArray = stock_transaction_record($symbol, $start_date);
...
function historical_stock_return($symbol, $start_date) {
		$perfArray = array();
		$count = 0;
		
		// get close price for symbol on starting date
		$query  = "select close from price_history where symbol = '".$symbol."' and trade_date = ";
		$query .= "(select min(trade_date) from price_history where symbol = '".$symbol."' and trade_date >= '".$start_date."')";
		$result = queryMysql($query);
		while ($data = mysql_fetch_row($result)) {
			$startPrice = $data[0];
		}
		...
		$query .= "from price_history where symbol = '".$symbol."' and trade_date > '".$start_date."'";		
...
function historical_stock_price($symbol, $start_date) {
		$perfArray = array();
		$count = 0;
		...
		// get performance of stock for each trade date compared to price on starting day
		$query  = "select trade_date, close, ((close - ".$startPrice.")/".$startPrice." * 100) ";
		$query .= "from price_history where symbol = '".$symbol."' and trade_date > '".$start_date."'";		
...
function turtle_portfolio_buy($date, $breakOutSignal, $ADX_filter, $breakOutOrderBy) {
		...
		$breakOutStockArray = getBreakoutStock2 ($date, $breakOutSignal, $breakOutOrderBy);
...
function turtle_portfolio_pyramid_buy ($date) { 
		...
		$my_sql = "select a.symbol, a.shares, c.high, c.trade_date, a.stop_loss, a.stop_buy, c.ATR, a.cost_basis, a.risk, a.risk_pct from turtle_portfolio a, price_history c where a.symbol = c.symbol  and c.trade_date = '".$date."' and c.high > a.stop_buy order by c.pct_change desc";
...
function getBreakoutStock4 ($today_date, $movingAvg, $rankAndWeightArray) {
	...
	/*		$query  = "select a.symbol, trade_date, high, low, close, daily_change, pct_change, ATR, 55_DAY_HIGH, 20_DAY_HIGH, 50_MA, 200_MA ";
			...
			$query .= " order by ".$rankBy." desc";
*/
			$query  = "select c.symbol, c.trade_date, c.open, c.high, c.low, c.close, c.daily_change, c.pct_change, ";
