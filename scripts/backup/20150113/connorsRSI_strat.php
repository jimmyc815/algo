...
if ($showOutput) {
	print "trade date: $trade_date <br />";
			...
			crsi_buy($trade_date, $breakOutSignal, $ADX_filter, $breakOutOrderBy, $portfolioID, $dailyBuyList, $enterCRSI, $enterRange, $enterLimit);
...
/*		
		$query  = "select a.symbol, b.close, b.low, b.daily_change, b.pct_change, b.ATR, b.55_DAY_HIGH, b.20_DAY_HIGH, b.20_DAY_LOW, b.10_DAY_LOW, ";
		$query .= "b.50_MA, b.200_MA, stop_loss, stop_buy, shares, risk, risk_pct, b.high, b.vsSpyRank, b.crsi, b.open ";
		$query .= "from turtle_portfolio a, quotes b, quotes c ";
		$query .= "where a.symbol = b.symbol and b.symbol = c.symbol ";
		$query .= "and a.portfolio_id = ".$portfolioID." ";
		$query .= "and a.symbol != 'CASH' ";
		$query .= "and c.crsi > $sell_crsi ";
...
function turtle_portfolio_pyramid_buy ($date, $portfolioID) { 
		...
		$my_sql = "select a.symbol, a.shares, c.high, c.trade_date, a.stop_loss, a.stop_buy, c.ATR, a.cost_basis, a.risk, a.risk_pct, c.low, c.open, c.vsSpyRank from turtle_portfolio a, quotes c where a.symbol = c.symbol  and c.trade_date = '".$date."' and c.high > a.stop_buy and a.portfolio_id = ".$portfolioID." order by c.pct_change desc";
...
function crsi_buy ($date, $breakOutSignal, $ADX_filter, $breakOutOrderBy, $portfolioID, $dailyBuyList, $enterCRSI, $enterRange, $enterLimit) {
		global $max_num_holdings;
		global $max_risk;
		global $risk_factor;
		global $stop_loss_muptiplier;
...
//		$breakOutStockArray = getBreakoutStock5 ($date, $breakOutSignal, $breakOutOrderBy, $portfolioID, $dailyBuyList);
//		$breakOutStockArray = getBreakoutStock6 ($date, $breakOutSignal, $breakOutOrderBy, $portfolioID, $dailyBuyList);
		$breakOutStockArray = getBuyList ($date, $enterCRSI, $enterRange, $portfolioID, $dailyBuyList, $enterLimit); 
...
function getBuyList ($today_date, $crsiThreshold, $enterRange, $portfolioID, $dailyBuyList, $pctLimitBelow) {
		global $simplePriceHistory;		
