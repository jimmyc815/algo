...
if ($showOutput) {
	print "trade date: $trade_date <br />";
			...
			crsi_sell ($trade_date, $portfolioID, $tranHistory, $exitCRSI, $stopLoss);
			crsi_buy($trade_date, $breakOutSignal, $ADX_filter, $breakOutOrderBy, $portfolioID, $dailyBuyList, $enterCRSI, $enterRange, $enterLimit, $tranHistory, $orderBy);
...
function crsi_sell ($date, $portfolioID, $tranHistory, $exitCRSI, $stopLoss) {
		...
		if ($stopLoss) {
			$query .= "union
						...
						and a.symbol != 'CASH' 
						and ((b.close - a.cost_basis) * 100 / a.cost_basis ) < $stopLoss			
...
function crsi_buy ($date, $breakOutSignal, $ADX_filter, $breakOutOrderBy, $portfolioID, $dailyBuyList, $enterCRSI, $enterRange, $enterLimit, $tranHistory, $orderBy) {
		global $max_num_holdings;
		global $max_risk;
		global $risk_factor;
		global $stop_loss_muptiplier;
...
*/
	            ...
	            if ($breakOutStockArray[$x]['buy_price'] > $breakOutStockArray[$x]['open']) {
		            ...
		            $p1 = ($breakOutStockArray[$x]['open'] + $breakOutStockArray[$x]['low'])/2;
		            ...
		            $gapdown = true;
					#print "in setting buy price: trade date: $date buy price > open: $s open price: $o buy price = $b purchase price = $p close price: $c P1: $p1 low: $l <br /> \n";
...
#print "date: $date symbol: ".$breakOutStockArray[$x]['symbol']." current risk :$current_risk \n";
				...
				if (($cash > $purchase_value) && ($current_risk < $max_risk)) {
					...
					if ($gapdown) {
						#print "actual buying trade date: $date buy price > open: $s open price: $o buy price = $b purchase price = $p close price: $c P1: $p1 low: $l <br /> \n";
						$gapdown = false;
					}
