...
//		$breakOutList = getBreakoutStock4 ($date, "55_DAY_HIGH", $testArray);
	...
	}   elseif($_GET['action'] == 'populate_quotes_memory_filter'){ 
		$portfolioID = $_GET['portfolio_id'];		
		...
		populate_quotes_memory_filter($portfolioID, $start_date, $end_date, $enterCRSI, $exitCRSI);
	}
...
function crsi_sell ($date, $portfolioID, $tranHistory, $exitCRSI, $stopLoss, $next_date) {
		...
		$query  = "select a.symbol, adj_open, adj_low, close, ATR, crsi, adj_close, enter_range, shares  ";
		$query .= "from $portfolio_table a, quotes_memory_filter b ";
		$query .= "where a.symbol = b.symbol ";
		$query .= "and a.portfolio_id = ".$portfolioID." ";
		$query .= "and a.symbol != 'CASH' ";
		$query .= "and b.crsi > $exitCRSI ";
		...
		while ($data = mysql_fetch_row($result)) {
			...
			$ph[$i]['adj_open'] = str_replace("\"", "",$data[1]);
			$ph[$i]['adj_low'] = str_replace("\"", "",$data[2]);
			$ph[$i]['close'] = str_replace("\"", "",$data[3]);
			$ph[$i]['ATR'] = str_replace("\"", "",$data[4]);
			$ph[$i]['crsi'] = str_replace("\"", "",$data[5]);
			$ph[$i]['adj_close'] = str_replace("\"", "",$data[6]);
			$ph[$i]['enter_range'] = str_replace("\"", "",$data[7]);
			$ph[$i]['shares'] = str_replace("\"", "",$data[7]);
...
#echo "cash: $cash purchase value: $purchase_value current risk: $current_risk max risk: $max_risk ", PHP_EOL;
...
function populateDailyBuyList ($today_date, $crsiThreshold, $enterRange, $portfolioID, $dailyBuyList, $pctLimitBelow) {
			...
			$query .= " from quotes_memory a, stock_list_memory b ";
			$query .= "where a.trade_date = '".$today_date."' ";
...
#echo "before crsi sell ", PHP_EOL;			
			crsi_sell ($trade_date, $portfolioID, $tranHistory, $exitCRSI, $stopLoss, $next_trade_date);
#echo "before crsi buy ", PHP_EOL;
			crsi_buy($trade_date, $breakOutSignal, $ADX_filter, $breakOutOrderBy, $portfolioID, $dailyBuyList, $enterCRSI, $enterRange, $enterLimit, $tranHistory, $orderBy, $next_trade_date, $skipFactor, $stop_loss_multiplier, $max_risk, $risk_factor);
#echo "after crsi buy ", PHP_EOL;			
...
function populate_quotes_memory_filter($portfolioID, $start_date, $end_date, $enterCRSI, $exitCRSI){
		...
		for ($x = 0; $x < $dateCount-1; $x++){
			...
			#echo "current trade_date: ", $trade_date, PHP_EOL;

			$query = "insert ignore into quotes_memory_filter (symbol, trade_date, adj_open, adj_low, close, ATR, crsi, adj_close, enter_range)
					  select symbol, 
					  		 trade_date, 
					  		 round(open*(adj_close/close), 2), 
					  		 round(low*(adj_close/close), 2), 
					  		 close, 
					  		 ATR, 
					  		 crsi, 
					  		 adj_close, 
					  		 round((close-low)*100/(high-low), 2)
					  from quotes_memory 
					  where trade_date = '$trade_date' 
					    and crsi > 0 
					    and symbol not in (select symbol from quotes_memory where trade_date = '$trade_date' and crsi > $enterCRSI and crsi < $exitCRSI)
			";		
