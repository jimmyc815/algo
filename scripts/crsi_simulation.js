...
*/
		...
		$("#datepicker_begin").val("3/2/2013");
		$("#datepicker_end").val("3/2/2014");
		...
		$("#date_range_6_month").click(function(e) {
		...
		$("#instrument_box").val("AAPL");

		$('#enterCRSI').click(function(e) {	
			enterCRSI = $("#enterCRSI").val();
		});		
		$('#enterRange').click(function(e) {	
			enterRange = $("#enterRange").val();
		});		
		$('#enterLimit').click(function(e) {	
			enterLimit = $("#enterLimit").val();
		});		
		$('#exitCRSI').click(function(e) {	
		...
		$('#maxRisk').click(function(e) {	
			maxRisk = $("#maxRisk").val();
		});		
		$('#riskFactor').click(function(e) {	
			riskFactor = $("#riskFactor").val();
		});		
		$('#riskSD').click(function(e) {	
			riskSD = $("#riskSD").val();
		});		
...
//portfolio_id | symbol | last_price | shares | cost_basis | overall_return | stop_loss | stop_buy | risk    | risk_pct |
	...
	function display_transaction() {
			var textInput = "select symbol, trade_type, trade_date, shares, price, vsSpyRank from turtle_portfolio_transaction where portfolio_id = 1 order by trade_date desc";
...
//portfolio_id | symbol | last_price | shares | cost_basis | overall_return | stop_loss | stop_buy | risk    | risk_pct |
	...
	function display_daily_buy_list() {
			var textInput = "select symbol, trade_date, buy_price, rank from turtle_daily_buy_list where portfolio_id = 1 order by rank asc";
	...
	function get_final_simulate_portfolio_value() {
		...
		{
			type: "GET",
            url: "scripts/portfolio_selection.php?action=get_historical_turtle_portfolio_value&date="+endingDate,
			async: false,
			data: "",
			dataType: "json",
			success: function(portfolioValue)
	...
	function display_transaction_p_and_l() {			
           	...
           	jQuery("#transactionPandL").jqGrid({
                	...
                	//url: "scripts/portfolio_selection.php?action=getDynamicSQLResult&txtInputQuery="+textInput,
                	url: "scripts/portfolio_selection.php?action=getTransactionPandL&portfolio_id=1",
                	datatype: 'json',
                	mtype: 'GET',
   ...
   // 	setTimeout(function() {
	...
	function startSimulation() {
	    ...
	    var startingTS = startingUTCDate.getTime();
	    
		var previousDate = new Date();
																
		var enterCRSI = ($("#enterCRSI").val());
		var enterRange = ($("#enterRange").val());
		var enterLimit = ($("#enterLimit").val());
		var exitCRSI = ($("#exitCRSI").val());
		var instrument = ($("#instrument_box").val());
		...
		//var startyear = year;
		//var startmon = mon;
		//var startday = startingDate.getDate();
		...
		var endingTS = endingUTCDate.getTime();
		...
		{
			type: "GET",
			url: "scripts/crsi_control.php?action=get_crsi&symbol="+instrument+"&startdate="+startingDate+"&enddate="+endingDate+"&enterCRSI="+enterCRSI+"&enterRange="+enterRange+"&enterLimit="+enterLimit+"&exitCRSI="+exitCRSI,
			data: "",
			timeout: 100000,
			//async: false,
			dataType: "json",
		...
		//alert ('ajax: ' + ajaxResult.responseText);
		$("#simOutput").val(ajaxResult.responseText);
	...
	function startSimulationFast() {
		...
		{
			type: "GET",
			url: "scripts/portfolio_selection.php?action=get_historical_stock_return&symbol=SPY&start_date="+startingDate+"&end_date="+endingDate,
			data: "",
			async: false,
			dataType: "json",
		...
		{
			type: "GET",
			url: "scripts/portfolio_selection.php?action=simulate_range_trade&start_date="+startingDate+"&end_date="+endingDate+"&breakoutSignal="+breakoutSignal+"&adx_filter="+adxFilter+"&breakoutOrderBy="+breakoutOrderBy+"&valueOrderByPctChange="+valueOrderByPctChange+"&valueOrderByRelVol="+valueOrderByRelVol+"&valueOrderByVsSpy="+valueOrderByVsSpy+"&maxRisk="+maxRisk+"&riskFactor="+riskFactor+"&riskSD="+riskSD,
			data: "",
			dataType: "json",
			success: function(portfolioReturn)
			{
...
//				$("#minReturn").val(data.min);
		...
		var processTimePerTradeDay = 300;
		var numDays;
		var refreshCount = 1;
		...
		{
			type: "GET",
			url: "scripts/portfolio_selection.php?action=get_num_of_trade_days&start_date="+startingDate+"&end_date="+endingDate,
			data: "",
			async: false,
			dataType: "json",
