...
if ($_GET['action'] == 'turtle_live_initial_setup') {
		...
		if (!$original_investment) $original_investment=100000;
		if (!$max_num_holdings) $max_num_holdings = 10;
...
//		$spyReturn = array();
		...
		refresh_portfolio_price($portfolioID);
/*		
...
//		}
	...
	update_benchmark_performance($portfolioID, "SPY");
...
} else if ($_GET['action'] == 'test_refresh_portfolio_price'){
	global $portfolioID ;

	$portfolioID = $_GET['portfolio_id'];

	refresh_portfolio_price($portfolioID);	
} else if ($_GET['action'] == 'test_get_init_setting'){
	global $portfolioID ;
	$tmp = array();
	
	$portfolioID = $_GET['portfolio_id'];

	$tmp = get_portfolio_initial_setting($portfolioID);	
...
} elseif ($_GET['action'] == 'test_update_benchmark') {
		update_benchmark_performance(2, 'SPY');

} elseif ($_GET['action'] == 'get_portfolio_value') {
		...
		//return $value;
} elseif ($_GET['action'] == 'get_portfolio_risk') {
		$value = get_realtime_turtle_portfolio_risk(2);
		//print "value: $value \n";
		//print "portfolio value: $value \n";
		$tmp[0]['prisk']=$value;
		echo json_encode($tmp);
...
} elseif($_GET['action'] == 'get_real_time_price'){ 		
		$symbol = $_GET['symbol'];

		$priceArray = array();

		
		$query = "select last_price from detail_quote where symbol = '".$symbol."' ";
		$result = queryMysql($query);
...
} elseif($_GET['action'] == 'get_real_time_price_change'){ 		
		$symbol = $_GET['symbol'];

		$priceArray = array();

		
		$query = "select percent_change from detail_quote where symbol = '".$symbol."' ";
		$result = queryMysql($query);

		while ($data = mysql_fetch_row($result)) {
			$priceArray[0]['percent_change'] = $data[0];
		}
		
		echo json_encode($priceArray);
} elseif($_GET['action'] == 'get_unrealized_gain_loss'){ 		
		$pid = $_GET['portfolio_id'];

		$priceArray = array();

		$priceArray[0]['unrealized_gain_loss'] = calculate_unrealized_gain_loss($pid);
		
		echo json_encode($priceArray);
} elseif($_GET['action'] == 'get_today_portfolio_change'){ 		
		$priceArray = array();

		$portfolioID = $_GET['portfolio_id'];
		
		$yesterday_pvalue = get_yeseterday_portfolio_value($portfolioID);
		$current_pvalue = get_realtime_turtle_portfolio_value($portfolioID);

		$priceArray[0]['daily_change'] = ($current_pvalue - $yesterday_pvalue) ;
		
/*		
		$query  = "select a.shares * b.daily_change from turtle_portfolio a, detail_quote b where a.symbol = b.symbol ";
		$query .= "and a.symbol != 'CASH' and a.portfolio_id = ".$portfolioID; 
...
*/
		...
		echo json_encode( $betaArray);
} else if ($_GET['action'] == 'get_cumulative_portfolio_return'){
	$pid = $_GET['portfolio_id'];
	
	$today_date = date("Y-m-d");  
	$retArray = array();
	
	$query = "select trade_date, return_pct from turtle_portfolio_performance where portfolio_id=".$pid." order by 1 ";
...
} else if ($_GET['action'] == 'get_cumulative_benchmark_return'){
	$pid = $_GET['portfolio_id'];
	
	$today_date = date("Y-m-d");  
	$retArray = array();
	
	$query = "select trade_date, overall_return from benchmark_performance where portfolio_id=".$pid." order by 1 ";
...
} else if ($_GET['action'] == 'get_daily_portfolio_return'){
	$pid = $_GET['portfolio_id'];
	
	$today_date = date("Y-m-d");  
	$retArray = array();
	
	$query = "select trade_date, day_return_pct from turtle_portfolio_performance where portfolio_id=".$pid." order by 1 ";
...
} else if ($_GET['action'] == 'get_daily_benchmark_return'){
	$pid = $_GET['portfolio_id'];
	
	$today_date = date("Y-m-d");  
	$retArray = array();
	
	$query = "select trade_date, day_return_pct from benchmark_performance where portfolio_id=".$pid." order by 1 ";
...
} else if ($_GET['action'] == 'get_realized_gain_loss') {
	$pid = $_GET['portfolio_id'];

	$retArray = array();
	
	$query = "select sum(gain_loss) from turtle_portfolio_transaction where portfolio_id = ".$pid;
	$result = queryMysql($query);

	while ($data = mysql_fetch_row($result)) {
			$realized_gain_loss = $data[0];
			$retArray[0]['realized_gain_loss'] = $data[0];
...
function live_turtle_portfolio_sell ($date, $portfolioID) {
		...
		$query  = "select a.symbol, b.last_price, daily_change, percent_change, stop_loss, stop_buy, shares, risk, risk_pct, last_trade_date, last_trade_time, cost_basis ";
...
//print $query;
		...
		while ($data = mysql_fetch_row($result)) {
			...
			$ph[$i]['cost_basis'] = str_replace("\"", "",$data[11]);
		...
		for ($x=0; $x < $i; $x++) {
			...
			$cost_basis = $ph[$x]['shares'] * $ph[$x]['cost_basis'];
			
			$gain_loss = $stock_sales - $cost_basis;
			...
			$insert_transaction_history = "insert into turtle_portfolio_transaction values (".$portfolioID.", '".$ph[$x]['symbol']."', 'SELL', '".$date."', ".$ph[$x]['shares'].", ".$sell_price.", ".$ph[$x]['risk'].", ".$ph[$x]['risk_pct'].", '".$ph[$x]['last_trade_time']."', $gain_loss, null)";
...
function live_turtle_portfolio_pyramid_buy ($date, $portfolioID) { 
		...
		while ($data = mysql_fetch_row($result)) {
			...
			$current_risk = get_realtime_turtle_portfolio_risk($portfolioID);
			...
			if (($cash > $purchase_value) && ($current_risk < $max_risk)) {
				...
				$query4  = "insert into turtle_portfolio_transaction (portfolio_id, symbol, trade_type, trade_date, shares, price, risk, risk_pct, trade_time) ";
...
*/
				...
				if (($cash > $purchase_value) && ($current_risk < $max_risk)) {
					...
					$result = queryMysql($my_sql);
					// insert into transaction history table
					$my_sql  = "insert into turtle_portfolio_transaction (portfolio_id, symbol, trade_type, trade_date, shares, price, risk, risk_pct, trade_time) ";
					$my_sql .= "values (".$portfolioID.", '".$breakOutStockArray[$x]['symbol']."', ";
					$my_sql .= "'BUY', ";
					$my_sql .= "'".$date."', ";
					$my_sql .= $num_shares.", ";
					...
					$my_sql .= $risk_dollar.", ";
					$my_sql .= $risk_pct.", ";
					$my_sql .= "'".$breakOutStockArray[$x]['last_trade_time']."' )";
					$result = queryMysql($my_sql);
					...
					$email_subject = $breakOutStockArray[$x]['last_trade_date']." ".$breakOutStockArray[$x]['last_trade_time']." Buy ".$breakOutStockArray[$x]['symbol']." ".$num_shares." shares at $".$breakOutStockArray[$x]['purchase_price'];
					$email_body = "";
					sendemail ($email_subject, $email_body);
...
function get_realtime_turtle_portfolio_risk($portfolioID) {
	global $portfolioID;
	
	$my_sql  = "select sum(risk_pct) from turtle_portfolio a where a.portfolio_id = ".$portfolioID." and a.symbol != 'CASH' ";
//print "my sql: $my_sql \n";
	$result = queryMysql($my_sql);
	while ($data = mysql_fetch_row($result)) {
		$value = $data[0];
...
function update_daily_portfolio_performance($portfolioID) {
	...
	$init_setting = array();
	$init_setting = get_portfolio_initial_setting($portfolioID);

	
	$value = get_realtime_turtle_portfolio_value($portfolioID);
	//$preturn = ($value - $original_investment) / $original_investment * 100;
	$preturn = ($value - $init_setting[0]['initial_investment']) / $init_setting[0]['initial_investment'] * 100;

	//$dollar_return = $value - $original_investment;
	$dollar_return = $value - $init_setting[0]['initial_investment'];
	
	$yesterday_pvalue = get_yeseterday_portfolio_value($portfolioID);
	
	$today_return = $value - $yesterday_pvalue;
	$today_return_pct = ($today_return / $yesterday_pvalue) * 100;

	$query2 = "insert into turtle_portfolio_performance values (".$portfolioID.", '".$today_date."', ".$dollar_return.", ".$preturn.", ".$value.", ".$today_return.", ".$today_return_pct." )";
...
function live_get_breakout_stock ($movingAvg, $rankAndWeightArray, $portfolioID, $dailyBuyList) {
		...
		//{		
			...
			*/
			$query  = "select d.rank, a.symbol, a.last_trade_date, a.last_price, a.percent_change, d.buy_price, a.last_trade_time ";
			$query .= "from detail_quote a, $dailyBuyList d ";
...
function send_end_of_day_email($portfolioID)
{
	...
	$query = "select trade_date, return_dollar, return_pct, portfolio_value from turtle_portfolio_performance where portfolio_id = ".$portfolioID." order by trade_date desc";
	$result = queryMysql($query);
	
	$email_body .= "<br>Past Performance<br>";
	$email_body .= "<table id='emailbody' border=1>";
	$email_body .= "<tr><td>Trade Date</td><td>Return in Dollar</td><td>Return Percentage</td><td>Portfolio Value</td></tr>";
	while ($data = mysql_fetch_row($result)) {
			if ($count == 0) {
				$email_subject .= $data[2]."%";
			}
			$email_body .= "<tr><td>".$data[0]."</td><td>".$data[1]."</td><td>".$data[2]."</td><td>".$data[3]."</td></tr>";
...
function refresh_portfolio_price($pid) {
	$query  = "update turtle_portfolio a, detail_quote b set a.last_price = b.last_price  ";
	$query .= ", a.profit_loss = a.shares * (b.last_price - a.cost_basis) ";
	$query .= ", a.overall_return = ((b.last_price - a.cost_basis) / a.cost_basis) * 100 ";
	$query .= "where a.symbol = b.symbol and ";
	$query .= "a.symbol != 'CASH' and ";
	$query .= "a.portfolio_id = $pid ";
...
function get_portfolio_initial_setting($pid)
{
	$init_setting = array();
	
	$query = "select portfolio_start_date, initial_investment, max_risk, max_num_holding, breakout_signal  ";
	$query .= "from turtle_portfolio_init_setup where portfolio_id = ".$pid;

	$result = queryMysql($query);
	
	while ($data = mysql_fetch_row($result)) {
			$init_setting[0]['portfolio_start_date'] = $data[0];
			$init_setting[0]['initial_investment'] = $data[1];
			$init_setting[0]['max_risk'] = $data[2];
			$init_setting[0]['max_num_holding'] = $data[3];
			$init_setting[0]['breakout_signal'] = $data[3];
	}
	
	return $init_setting;	
}

function calculation_transaction_gain_and_loss($pid, $symbol) 
{
	$query = "select trade_type, trade_date, shares, price from turtle_portfolio_transaction where portfolio_id = ".$pid;
	$query .= " and symbol = '".$symbol."' order by trade_date";
...
function update_benchmark_performance($pid, $benchmark) {
	// select initial benchmark price
	$query = "select price from benchmark_performance where trade_date = (select min(trade_date) from benchmark_performance where portfolio_id = ".$pid.")";

	$result = queryMysql($query);
	while ($data = mysql_fetch_row($result)) {
		$initial_benchmark = $data[0];
	}
	
	$query  = "insert into benchmark_performance (portfolio_id, symbol, trade_date, price, day_return, overall_return, day_return_pct) select ";
	$query .= $pid.", symbol, last_trade_date, last_price, daily_change, (last_price - ".$initial_benchmark.")*100/".$initial_benchmark.", percent_change from detail_quote where ";
	$query .= "symbol = '".$benchmark."'";

	$result = queryMysql($query);	
	
}

function calculate_unrealized_gain_loss($pid) {
	// select initial benchmark price
	$query = "select sum((shares*last_price) - (shares*cost_basis)) as unrealized_gain_loss from turtle_portfolio where portfolio_id = ".$pid;
...
function get_yeseterday_portfolio_value($pid) {
		$today_date = date("Y-m-d");  

		$query  = "select portfolio_value from turtle_portfolio_performance where portfolio_id = ".$pid;
		$query .= " and trade_date = (select max(trade_date) from turtle_portfolio_performance where portfolio_id = ".$pid;
		$query .= " and trade_date < '".$today_date."') ";
//		$query .= " and trade_date < '2013-02-22') ";

		$result = queryMysql($query);

		while ($data = mysql_fetch_row($result)) {
			//$priceArray[0]['portfolio_value'] = $data[0];
			$pvalue = $data[0];
		}
