...
//		$breakOutList = getBreakoutStock4 ($date, "55_DAY_HIGH", $testArray);
		...
		#$orderBy = preg_replace('/^(\'(.*)\'|"(.*)")$/', '$2$3', $orderBy);
		
		
		echo "order by : $orderBy ", PHP_EOL;
		...
		echo "pid: $portfolioID	start date: $start_date  end date: $end_date enter crsi: $enterCRSI	enter range: $enterRange enter limit: $enterLimit exit crsi: $exitCRSI commission: $commission	max risk: $max_risk	risk factor: $risk_factor stop_loss mult: $stop_loss_multiplier order by: $orderBy skip factor: $skipFactor ", PHP_EOL;
...
#echo "date: $date cash: $cash purchase value: $purchase_value current risk: $current_risk max risk: $max_risk ", PHP_EOL;
...
echo "BUY: ", $buy_list[$buy_count]['symbol'], " next open: ", $buy_list[$buy_count]['next_open'], " crsi: ", $buy_list[$buy_count]['crsi'], PHP_EOL;
...
/*$query = "select trade_date, symbol, buy_price from $dailyBuyList ";
echo "query: $query ", PHP_EOL;
$result = queryMysql($query);
while ($row = mysql_fetch_row($result)) {
	echo "date : ", $row[0], " symbol: ", $row[1], " buy price: ", $row[2], PHP_EOL;
}
*/
...
function simulate_range_trade($portfolioID, $start_date, $end_date, $enterCRSI, $enterRange, $enterLimit, $exitCRSI, $commission, $max_risk, $risk_factor, $stop_loss_multiplier, $orderBy, $skipFactor ) {
		...
		echo "trade date: $trade_date 	breakoutsignal: $breakOutSignal	adx flter: $ADX_filter order by: $breakOutOrderBy	pid: $portfolioID dailybuylist: $dailyBuyList enterCRSI: $enterCRSI exitCRSI: $exitCRSI enterRange: $enterRange	enterLimit: $enterLimit	tranHist: $tranHist orderBy: $orderBy next trade date: $next_trade_date skip factor: $skipFactor stop loss: $stop_loss_multiplier max risk: $max_risk risk factor: $risk_factor next trdade date: $next_trade_date ", PHP_EOL;
