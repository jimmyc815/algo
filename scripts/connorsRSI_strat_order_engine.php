...
function crsi_sell ($date, $portfolioID, $tranHistory, $exitCRSI, $stopLoss, $next_date) {
		...
		for ($x=0; $x < $i; $x++) {			
			...
			$sell_order = array();
			$sell_order['buy_sell']='sell';
			$sell_order['order_type']='open';
			$sell_order['symbol']=$ph[$x]['symbol'];
			$sell_order['order_date']=$date;
			$sell_order['shares']=$ph[$x]['shares'];
			
		    array_push($order_queue, $sell_order);	
...
if ($showOutput) {
	print "insert transaction: $insert_transaction_history <br /> ";
		...
		order_management_engine($date, $next_date);		
...
//		echo json_decode($breakOutStockArray);	
		...
		for ($x=1; $x < $len_array; $x++) {
			...
			$buy_order = array();
			$buy_order['buy_sell']='buy';
			$buy_order['order_type']='open';
			$buy_order['symbol']=$breakOutStockArray[$x]['symbol'];
			$buy_order['order_date']=$date;
			$buy_order['shares']=$num_shares;
			
		    array_push($order_queue, $buy_order);	
			
		}

		#foreach ($order_queue as $current_row) {
		#	foreach ($current_row as $rowkey => $rowvalue) {
		#		print "key: $rowkey value: $rowvalue  ";
		#	}	
		#print "\n";
		#}
...
function order_management_engine($today, $tomorrow){
	global $order_queue;
	
	$current_order_len = sizeof($order_queue);
	
	$proceeds = 0;
	$cost = 0;
	$sell_queue = array();
	$sell_open_queue = array();
	$buy_queue = array();
	
	
	print "order queue length: $current_order_len \n";

	for ($x=0; $x<$current_order_len; $x++){
		## pop FIFO first element off order queue
		$each_order = array_shift($order_queue);
		
		if ($each_order['buy_sell'] == 'buy') {
			array_push($buy_queue, $each_order);
			
		} elseif ($each_order['buy_sell'] == 'sell') {
			$order_type = $each_order['order_type'];
			$order_date = $each_order['order_date'];
			$symbol = $each_order['symbol'];
			$shares = $each_order['shares'];
			
			array_push($sell_open_queue, $each_order);
			print "sell \n";
		}

		foreach ($each_order as $rowkey => $rowvalue) {
			print "key: $rowkey value: $rowvalue  ";
		}	
	...
	if ($sell_open_queue) {
		foreach ($sell_open_queue as $each_sell) {
			if ($count > 0) { 
				$symbol_list .= ", ";
			}
			$count++;
			
			$symbol_list .= " '".$each_sell['symbol']."'";
		}	
		
		$query = "select symbol, open * (adj_close / close) from quotes where trade_date = '".$tomorrow."' and symbol in (".$symbol_list.")";
